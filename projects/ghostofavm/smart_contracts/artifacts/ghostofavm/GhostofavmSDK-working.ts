...
class GhostBase {
  ...
  public cacheParamsTimeout = 13 // ms
...
export class GhostofavmSDK extends GhostBase {
  ...
  async acctBalanceData(
    ...
    const arg0 = Array.isArray(argsOrArgsArray) ? argsOrArgsArray[0] : argsOrArgsArray
    const {
      transactions: [transactionForArgs],
    } = await this.client.createTransaction.acctBalanceData({ args: arg0, ...extraMethodCallArgs })
    const txnArgs = {
      ...
      suggestedParams,
      appArgs: transactionForArgs.applicationCall!.appArgs as any,
    }
    console.log({txnArgs})
    const transaction = await makeApplicationCreateTxnFromObject(txnArgs)
    return this.execute({
      transactions: [transaction],
